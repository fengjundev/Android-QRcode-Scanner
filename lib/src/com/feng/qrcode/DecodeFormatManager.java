...
public final class DecodeFormatManager {
  ...
  static {
    ...
    FORMATS_FOR_MODE.put(QRcodeIntents.Scan.ONE_D_MODE, ONE_D_FORMATS);
    FORMATS_FOR_MODE.put(QRcodeIntents.Scan.PRODUCT_MODE, PRODUCT_FORMATS);
    FORMATS_FOR_MODE.put(QRcodeIntents.Scan.QR_CODE_MODE, QR_CODE_FORMATS);
    FORMATS_FOR_MODE.put(QRcodeIntents.Scan.DATA_MATRIX_MODE, DATA_MATRIX_FORMATS);
    FORMATS_FOR_MODE.put(QRcodeIntents.Scan.AZTEC_MODE, AZTEC_FORMATS);
    FORMATS_FOR_MODE.put(QRcodeIntents.Scan.PDF417_MODE, PDF417_FORMATS);
  }
  ...
  static Set<BarcodeFormat> parseDecodeFormats(Intent intent) {
    ...
    CharSequence scanFormatsString = intent.getStringExtra(QRcodeIntents.Scan.FORMATS);
    if (scanFormatsString != null) {
      scanFormats = Arrays.asList(COMMA_PATTERN.split(scanFormatsString));
    }
    return parseDecodeFormats(scanFormats, intent.getStringExtra(QRcodeIntents.Scan.MODE));
  }
